...
def trainTextRNN():
  ...
  x_train, xdev, y_trian，y_dev = train_test_split(x, y, test_size=0.1)
  ...
  with graph.as_default():
    ...
    with sess.as_default():
      textRNN_mdoel = TextRNN(
        num_class=y_train.shape[1],
        batch_size=params['batch_size'],
        sequence_length=x_train.shape[1],
        embedding_size=params['embedding_dim'],
        hidden_unit=params['hidden_unit'],
        vocab_size=vocabulary.shape[1],
        )
      ...
      for epoch in range(curr_epoch, params.num_epochs)
        ...
        for start, end in zip(range(0, number_of_training_data, batch_size), range(batch_size, number_of_training_data, batch_size)):
          if epoch == 0 and counter == 0:
            logging.info('x_train[{}:{}]: {}'.format(start, end, x_train[start:end]))
          curr_loss, curr_acc, _= sess.run([textRNN_mdoel.loss, textRNN_mdoel.accuracy, textRNN_mdoel.train_op], feed_dict={
            textRNN_mdoel.input_x: x_train[start:end], 
            textRNN_mdoel.input_y: y_train[start:end].
